...
void ExceptionHandler(ExceptionType which)
{
	...
	case SyscallException:
	{
		switch (type)
		{
			...
			// System call doc mot so nguyen do nguoi dung nhap
			case SC_ReadInt:
			{
				char* buffer;
				int MAX_BUFFER = 255;
			...
			// System call in mot so nguyen do nguoi dung nhap
			case SC_PrintInt:
			{
			...
			// System call doc mot ky tu do nguoi dung nhap
			case SC_ReadChar:
			{
				int maxBytes = 255;
			...
			// System call in mot ky tu do nguoi dung nhap
			case SC_PrintChar:
			{
			...
			// System call doc mot chuoi string do nguoi dung nhap
			case SC_ReadString:
			{
				int virtAddr, length;
				char* buffer;
			...
			// System call in mot chuoi string do nguoi dung nhap
			case SC_PrintString:
			{
				int virtAddr;
				char* buffer;
			...
			// system call mo 1 file va tra ve OpenFileID neu thanh cong
			case SC_Open:
			{
			...
			// system call dong file thong qua OpenFileID duoc truyen vao
			case SC_Close:
			{
			...
			// System call doc du lieu trong file 
			case SC_Read:
			{
			...
			// system call di chuyen vi tri cua con tro file
			case SC_Seek:
			{
		...
		IncreasePC();
		break;
		//Xu ly cac exceptions trong machine.h (cac loi runtime)

		// TH: NoExcepion -> tra quyen cho he dieu hanh
		case NoException:
		{
			return; //Tra quyen cho he dieu hanh
		}

		//TH: Bao loi va Halt he thong
		case PageFaultException:
		{
			printf("No valid translation found/%d %d occurs\n", which, type);
			interrupt->Halt();
		}
		...
		{
			printf("Unaligned reference or one that was beyond the endof the address space/%d %d occurs\n", which, type);
			interrupt->Halt();
		}
		case OverflowException:
		{
			printf("Interger overflow /%d %d occurs\n", which, type);
			interrupt->Halt();
		}
